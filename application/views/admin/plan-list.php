	...
	<script>
		...
		var get_plans = (function (user_id=null, page=null, count=null) {
					...
					if (response.status == 'ok') {
						...
						if(response.details.length > 0){
							...
							$.each(response.details, function( index, value ) {
								...
								var date = moment.unix(value.timestamp).format("DD-MMMM-YYYY");
								
								htmlData+= '<tr id="'+ value.user_id +'">'+
											'<td class="hide-this pagenow" data-val="'+ pagenow +'"></td>'+
											'<td>'+ uni_index +'</td>'+
											'<td>'+ value.product_title +'</td>'+
											'<td><a href="javascript:void(0)">'+ value.first_name +' '+ value.last_name +'</a></td>'+
